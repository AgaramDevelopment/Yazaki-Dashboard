        ...
        switch (self.textStyle) {
            case MCPieChartViewTextStyleRealValue:
            {
                slice.textLabel.text = [NSString stringWithFormat:@"%.0f", value];
                 ...
                 slice.textLabel.text =str;
                  [slice.textLabel setFont:[UIFont fontWithName:@"RAJDHANI-BOLD" size:17]];
            }
                break;
            case MCPieChartViewTextStylePercentage:
            {
                slice.textLabel.text = [NSString stringWithFormat:@"%.0f%%", percentage*100];
                NSString * str = [NSNumber numberWithFloat:percentage].stringValue;
                  slice.textLabel.text =str;
                  [slice.textLabel setFont:[UIFont fontWithName:@"RAJDHANI-BOLD" size:17]];
            }
                break;
...
//        switch (self.textStyle) {
//            case MCPieChartViewTextStyleRealValue:
//                slice.textLabel.text = [NSString stringWithFormat:@"%f", value];
//                
//                NSString * str = [NSNumber numberWithFloat:value].stringValue;
//            //  [slice setString:@""];
//                slice.textLabel.text =str;
...
//                slice.textLabel.text = [NSString stringWithFormat:@"%.0f%%", percentage*100];
//                
//                 NSString * str1 = [NSNumber numberWithFloat:percentage].stringValue;
//                  slice.textLabel.text =str1;
//                break;
//        }
//    }
